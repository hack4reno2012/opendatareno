...
class GFExport{
        ...
        if(isset($_POST["export_lead"])){
            ...
            foreach($forms as &$form){
                foreach($form["fields"] as &$field){
                    ...
                    if(in_array($inputType, array("checkbox", "radio", "select"))){
        ...
        if($_POST["import_forms"]){
            ...
            if($forms){
                if(version_compare($forms["version"], self::$min_import_version, "<")){
                    ...
                    $form_text = sizeof($forms) > 1 ? __("forms", "gravityforms") : __("form", "gravityforms");
                    ?>
                    <div class="updated" style="padding:10px;"><?php echo sprintf(__("Gravity Forms imported %d {$form_text} successfully", "gravityforms"), sizeof($forms)); ?></div>
                    <?php
                }
            }
            else{
    ...
    public static function export_lead_page(){
        ?>
        <script type='text/javascript' src='<?php echo GFCommon::get_base_url()?>/js/jquery-ui/ui.datepicker.js'></script>
                ...
                if(in_array($input_type, array("checkbox", "radio", "select"))){
                    ...
                    for($i=1, $count = sizeof($field["choices"]); $i<=$count; $i++){
                        ...
                        if($input_type == "checkbox")
                            $field["inputs"][] = array("id" => $i, "label" => $field["choices"][$i-1]["text"]);
                    }
