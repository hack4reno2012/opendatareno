...
class GFFormDisplay{
    ...
    public static function get_form($form_id, $display_title=true, $display_description=true, $force_display=false, $field_values=null){
        ...
        GFCommon::$tab_index = apply_filters("gform_tabindex_{$form_id}",apply_filters("gform_tabindex", 1, $form));
        ...
        if(empty($confirmation_message)){
            ...
            $form_string .= apply_filters("gform_form_tag", "<form method='post' enctype='multipart/form-data' id='gform_$form_id' class='" . $form["cssClass"] . "' action='{$action}'>", $form);
            ...
            $form_string .= "
                        ...
                        if($form["button"]["type"] == "text" || empty($form["button"]["imageUrl"])){
                            ...
                            $button_input = "<input type='submit' id='gform_submit_button_{$form["id"]}' class='button' value='" . esc_attr($button_text) . "' tabindex='$tabindex'/>";
                        }
                        else{
                            $imageUrl = $form["button"]["imageUrl"];
                            $button_input= "<input type='image' src='$imageUrl' id='gform_submit_button_{$form["id"]}' alt='submit' tabindex='$tabindex'/>";
                        }
    ...
    public static function handle_confirmation($form, $lead){
        if($form["confirmation"]["type"] == "message"){
            $anchor = apply_filters("gform_confirmation_anchor", 0) ? "<a name='gf_{$form["id"]}' class='gform_anchor' ></a>" : "";
    ...
    public static function send_emails($form, $lead){
        ...
        $to = apply_filters("gform_autoresponder_email_$form_id", apply_filters("gform_autoresponder_email", stripslashes($_POST["input_" . $form["autoResponder"]["toField"]]), $form));
                ...
                switch(RGFormsModel::get_input_type($field)){
                    ...
                    case "address" :
                        if($field["isRequired"])
                        {
                            ...
                            if(empty($street) || empty($city)|| empty($state) || empty($zip) || (empty($country) && !$field["hideCountry"])){
                                $field["failed_validation"] = true;
    ...
    private static function get_conditional_logic($form){
        ...
        jQuery(document).ready(function(){
            ...
            jQuery('#gform_{$form['id']}').submit(
                function(){
                    return jQuery('#gform_submit_button_{$form['id']}').is(':visible');
                }
            );
        });
